...
struct FastRNG {
    ...
    inline long double rf() { 
        return (next() >> 11) * 0x1.0p-53L; 
    }

    inline long double rf2() { 
        return rf() * 2.0L - 1.0L; 
    }
...
inline bool segInt(long double ax, long double ay, 
...
struct Cfg {
    ...
    inline void updAll() { 
        for (int i = 0; i < n; i++) upd(i); updGlobal(); 
    }
...
Cfg localSearch(Cfg c, int maxIter) {
    long double bs = c.side();
    const long double steps[] = {
        0.01L, 0.004L, 0.0015L, 0.0006L, 0.00025L, 0.0001L
    };
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (a=="-q" || a=="--quiet" || a=="--simple") simple=true;
    }

    int numThreads = omp_get_max_threads();
    if (simple) {
        printf("TreePacker v21 | threads=%d | iters=%d | restarts=%d\n", numThreads, iters, restarts);
        printf("Loading %s...\n", in.c_str());
    } else {
    ...
    for (int idx = 0; idx < (int)nvals.size(); idx++) {
        ...
        #pragma omp critical
        {
            res[n] = o;
            if (c_ovl && !o_ovl) {
                if (!simple) { printf("n=%3d: %.6Lf -> %.6Lf (FIXED OVERLAP, %.4Lf%%)\n", n, os, ns, (os-ns)/os*100.0L); fflush(stdout); }
                totalImproved++;
            } else if (o_ovl) {
                if (!simple) { printf("n=%3d: WARNING - still has overlap! (score %.6Lf)\n", n, ns); fflush(stdout); }
            } else if (ns < os - 1e-10L) {
                if (!simple) { printf("n=%3d: %.6Lf -> %.6Lf (%.4Lf%%)\n", n, os, ns, (os-ns)/os*100.0L); fflush(stdout); }
                totalImproved++;
            }
        }
    }
    ...
    while (changed && passNum < 10) {
        ...
        for (int k = 200; k >= 2; k--) {
            ...
            if (sideK < sideK1 - 1e-12L) {
                ...
                if (bestSide < sideK1 - 1e-12L && !bestCfg.anyOvl()) {
                    ...
                    res[k-1] = bestCfg;
                    if (!simple) { printf("n=%3d: %.6Lf -> %.6Lf (from n=%d removal, %.4Lf%%)\n", k-1, oldScore, newScore, k, (oldScore-newScore)/oldScore*100.0L); fflush(stdout); }
                    backPropImproved++;
                    changed = true;
                }
            }
            ...
            for (int src = k + 1; src <= min(200, k + 5); src++) {
                ...
                if (sideSrc < sideK - 1e-12L) {
                    ...
                    if (bestSide < sideK - 1e-12L && !bestCfg.anyOvl()) {
                        ...
                        long double newScore = bestCfg.score();
                        res[k] = bestCfg;
                        if (!simple) { printf("n=%3d: %.6Lf -> %.6Lf (from n=%d removal, %.4Lf%%)\n", k, oldScore, newScore, src, (oldScore-newScore)/oldScore*100.0L); fflush(stdout); }
                        backPropImproved++;
                        changed = true;
                    }
                }
        ...
        if (changed && !simple) printf("Pass %d complete, continuing...\n", passNum);
    }
    ...
    if (simple) {
        printf("Summary: initial=%.6Lf final=%.6Lf improve=%.6Lf (%.4Lf%%) phase1=%d phase2=%d time=%.1Lfs threads=%d\n",
               init, fin, init-fin, (init-fin)/init*100.0L, totalImproved, backPropImproved, el, numThreads);
    } else {
        printf("\n========================================\n");
        printf("Initial: %.6Lf\nFinal:   %.6Lf\n", init, fin);
