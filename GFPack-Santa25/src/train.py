...
def collectData():
    ...
    dirName = DATASET_DIR / "santa_train_data.pkl"
    ...
    # polyIds, actions = padData(polyIds, actions) # Removed padData as our generator produces correct scale
...
def collectValiData():
    ...
    dirName = DATASET_DIR / "santa_train_data.pkl" # Use same for now, or split
    dataFile = open(dirName, "rb")
    print("load data from ", dirName)
    polyIds = pickle.load(dataFile)
    actions = pickle.load(dataFile)
    
    # Simple split: take last 100 for validation
    polyIds = polyIds[-100:]
    actions = actions[-100:]
